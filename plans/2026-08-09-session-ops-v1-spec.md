...
**A direct commit is confined to the file it wrote.** The second consequence a push to `main` could have — firing a unit's deploy — is removed deterministically rather than architecturally: both prompts require every bot commit message to end with `[skip ci]`, which GitHub honours for `push` and `pull_request` events. This was the stated driver for draft-PR mode, and it does not need one; a branch that must never auto-merge (or the deploy fires on merge anyway) diverges from `main` without bound, and beads built and then removed that exact shape. Units outside those two events, or on a deploy platform that ignores the token, use `paths-ignore` on their own deploy workflow. What neither control reaches is a protected `main` forbidding direct pushes — that genuinely needs a PR, and is what SEQ-014 now tracks, gated on a unit having one. Evidence, not anticipation, per Appendix C's posture. Full analysis: `research/2026-08-13-ops-draft-pr-mode.md`.
